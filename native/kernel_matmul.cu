...
__device__ float kernel_function(
    const float x1,
    const float x2,
...
template <int kernel_function_type, int num_params>
__device__ std::array<float, num_params> kernel_function_bwd(
    const float x1,
    const float x2,
    const std::array<float, num_params> params
);
...
__device__ __forceinline__ float kernel_function<LOCALLY_PERIODIC, LOCALLY_PERIODIC_NUM_PARAMS>(
    const float x1,
    const float x2,
    const std::array<float, LOCALLY_PERIODIC_NUM_PARAMS> params
) {
...
template <>
__device__ __forceinline__ std::array<float, LOCALLY_PERIODIC_NUM_PARAMS> kernel_function_bwd<LOCALLY_PERIODIC, LOCALLY_PERIODIC_NUM_PARAMS>(
    const float x1,
    const float x2,
    const std::array<float, LOCALLY_PERIODIC_NUM_PARAMS> params
) {
    ...
    const float lengthscale_rbf_diff = value * diff_sq / (lengthscale_rbf * lengthscale_rbf * lengthscale_rbf);
    const float lengthscale_periodic_diff = value * periodic / lengthscale_periodic;
    const float period_length_diff = value * 2 * sin(2 * diff / period_length) * diff / (lengthscale_periodic * period_length * period_length);
    const float outputscale_diff = exp_term;

    return {
        lengthscale_rbf_diff,
        lengthscale_periodic_diff,
        period_length_diff,
        outputscale_diff
    };
}
...
template <>
__device__ __forceinline__ float kernel_function<RBF, RBF_NUM_PARAMS>(
    const float x1,
    const float x2,
...
template <>
__device__ __forceinline__ std::array<float, RBF_NUM_PARAMS> kernel_function_bwd<RBF, RBF_NUM_PARAMS>(
    const float x1,
    const float x2,
    const std::array<float, RBF_NUM_PARAMS> params
) {
...
__device__ __forceinline__ float kernel_function<SPECTRAL, SPECTRAL_NUM_PARAMS>(
    const float x1,
    const float x2,
    const std::array<float, SPECTRAL_NUM_PARAMS> params
) {
...
template <>
__device__ __forceinline__ std::array<float, SPECTRAL_NUM_PARAMS> kernel_function_bwd<SPECTRAL, SPECTRAL_NUM_PARAMS>(
    const float x1,
    const float x2,
    const std::array<float, SPECTRAL_NUM_PARAMS> params
) {
...
__global__ void kernel_matmul_cuda_kernel(
    ...
    // Index calculations
    static_assert(thread_dim * per_thread == block_size, "block_size must be the product of thread_dim and per_thread");
    static_assert((thread_dim * thread_dim) % 32 == 0, "Thread block must be evenly divisible in warps.");
    ...
    // This is an alternative indexing that is used for loading from global to shared memory to avoid bank conflicts.
    ...
    static_assert(thread_dim % warp_num == 0, "thread_dim must be evenly divisible by the number of warps.");
    ...
    for(int n_base = start_m; n_base < end_m; n_base += thread_dim) {
        ...
        for(int j = warp_based_y; j < thread_dim; j += warp_num) {
            for(int i = warp_based_x; i < block_size; i += 32) {
                ...
                if(m < m_size && n < end_m) {
                    shm_kernel[shm_index] = kernel_function<kernel_function_type, num_params>(
                        x1[m],
                        x2[n],
                        reg_params
                    );
...
__global__ void kernel_weighted_bilinear_derivative_cuda_kernel(
    ...
    // Write output to global memory
    for(int p = 0; p < num_params; p++) {
        params_grad[p][blockIdx.z][blockIdx.y][blockIdx.x][threadIdx.y][threadIdx.x] = weight * accumulator[p];
    }
    weights_grad[blockIdx.y][task1][task2][blockIdx.x][threadIdx.y][threadIdx.x] = weight_accumulator;
}
...
__global__ void kernel_matmul_cuda_kernel_bwd(
    ...
    // This is almost the same as the forward pass, as we have the same matmul structure in the derivative.
    // The main difference is that we can now accumulate the gradients to smaller portions directly, saving registers.
    // For this, we need the output gradient, which we will load from global memory directly and cache in registers.
    // However, we will need significantly more shm and registers as we now need to buffer all four gradients of the
    // kernel function.
    ...
    for(int n_base = start_m; n_base < end_m; n_base += thread_dim) {
        ...
        for(int i = 0; i < thread_dim; i++) {
            ...
            for(int j = 0; j < per_thread; j++) {
                ...
                #pragma unroll
                for(int p = 0; p < num_params; p++) {
                    reg_params_grad[p][j] = shm_params_grad[p * buffer_size + shm_index + threadIdx.y];
                }
            }
...
template <int block_size, int thread_dim, int per_thread, int kernel_function_type, int num_params, int k_block_size>
...
__global__ void lmc_matmul_cuda_kernel(
    ...
    for(int n = n_start; n < n_end; n++) {
        ...
        for(int l = 0; l < params.size(1); l += num_threads_tasks1) {
            ...
            if(num_threads_tasks1 > 1) {
                if(l_global < params.size(1)) {
                    ...
                    data_covar[threadIdx.y][threadIdx.x] = kernel_function<kernel_function_type, num_params>(
                        x1_reg,
                        x2_n,
                        params_reg
                    );
                ...
                if(l_global < params.size(1)) {
                    ...
                    data_covar_reg[0] = kernel_function<kernel_function_type, num_params>(
                        x1_reg,
                        x2_n,
                        params_reg
                    );
        ...
        // TODO: For K = 1 (or maybe even K < M), it might be good to merge phases I and II to reduce register usage
        for(int t2 = 0; t2 < num_tasks; t2++) {
...
torch::Tensor kernel_matmul_cuda(
    ...
    if(rhs.size(1) > 40) {
        ...
        const dim3 blocks {
            (rhs.size(1) + block_size - 1) / block_size,
        ...
        kernel_matmul_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params><<<blocks, threads, shared>>>(
        ...
        switch(rhs.size(1)) {
        case 1:
            kernel_matmul_vector_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params, 1><<<blocks, threads>>>(
                x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
            ...
            break;
        case 2:
            kernel_matmul_vector_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params, 2><<<blocks, threads>>>(
                x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
            ...
            break;
        case 3:
            kernel_matmul_vector_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params, 3><<<blocks, threads>>>(
                x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
            ...
            break;
        case 4:
            kernel_matmul_vector_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params, 4><<<blocks, threads>>>(
                x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
            ...
            break;
        case 5:
            kernel_matmul_vector_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params, 5><<<blocks, threads>>>(
                x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
            ...
            break;
        case 6:
            kernel_matmul_vector_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params, 6><<<blocks, threads>>>(
                x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
            ...
            break;
        case 7:
            kernel_matmul_vector_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params, 7><<<blocks, threads>>>(
                x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
            ...
            break;
        case 8:
            kernel_matmul_vector_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params, 8><<<blocks, threads>>>(
                x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
        ...
        case 11:
        case 12:
            kernel_matmul_vector_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params, 12><<<blocks, threads>>>(
                x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
        ...
        case 15:
        case 16:
            kernel_matmul_vector_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params, 16><<<blocks, threads>>>(
                x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
        ...
        case 19:
        case 20:
            kernel_matmul_vector_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params, 20><<<blocks, threads>>>(
                x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
        ...
        case 23:
        case 24:
            kernel_matmul_vector_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params, 24><<<blocks, threads>>>(
                x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
        ...
        case 27:
        case 28:
            kernel_matmul_vector_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params, 28><<<blocks, threads>>>(
                x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
        ...
        case 31:
        case 32:
            kernel_matmul_vector_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params, 32><<<blocks, threads>>>(
                x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
        ...
        case 35:
        case 36:
            kernel_matmul_vector_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params, 36><<<blocks, threads>>>(
                x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
        ...
        case 39:
        case 40:
            kernel_matmul_vector_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params, 40><<<blocks, threads>>>(
                x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
                rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
...
torch::Tensor kernel_bilinear_derivative_cuda(
    ...
    const auto out_opts = torch::TensorOptions()
        ...
        .layout(x1.layout())
        .device(x1.device());
    auto out = torch::zeros({params.size(0), params.size(1), blocks.x, thread_dim, thread_dim}, out_opts);

    #ifdef PRINT_SIZE
    printf("m, n, k, b: (%d, %d, %d, %d)\n", x1.size(0), x2.size(0), left_vecs.size(1), params.size(1));
    printf("threads: (%d, %d, %d)\n", threads.x, threads.y, threads.z);
    printf("blocks: (%d, %d, %d)\n", blocks.x, blocks.y, blocks.z);
    #endif
    ...
    kernel_bilinear_derivative_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params><<<blocks, threads>>>(
        x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
        x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
...
std::array<torch::Tensor, 2> kernel_weighted_bilinear_derivative_cuda(
    ...
    const auto out_opts = torch::TensorOptions()
        ...
        .layout(x1.layout())
        .device(x1.device());
    auto out = torch::zeros({params.size(0), blocks.z, blocks.y, blocks.x, thread_dim, thread_dim}, out_opts);
    auto out_weights = torch::zeros({blocks.y, num_tasks, num_tasks, blocks.x, thread_dim, thread_dim}, out_opts);

    #ifdef PRINT_SIZE
    printf("m, n, k, b, t: (%d, %d, %d, %d, %d)\n", x1.size(0), x2.size(0), left_vecs.size(1), params.size(1), num_tasks);
    printf("threads: (%d, %d, %d)\n", threads.x, threads.y, threads.z);
    printf("blocks: (%d, %d, %d)\n", blocks.x, blocks.y, blocks.z);
    #endif

    const auto left_vecs_t = left_vecs.view({x1.size(0), weights.size(1), left_vecs.size(1)}).permute({2, 1, 0}).contiguous();
    const auto right_vecs_t = right_vecs.view({x2.size(0), weights.size(2), right_vecs.size(1)}).permute({2, 1, 0}).contiguous();

    kernel_weighted_bilinear_derivative_cuda_kernel<block_size, thread_dim, per_thread, kernel_function_type, num_params><<<blocks, threads>>>(
        x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
        x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
...
torch::Tensor kernel_matmul_cuda_bwd(
    ...
    const auto out_opts = torch::TensorOptions()
        ...
        .layout(x1.layout())
        .device(x1.device());
    const auto out_shape = torch::IntArrayRef({num_params, params.size(1), x1.size(0), rhs.size(1)});
    auto params_grad = torch::zeros(out_shape, out_opts);

    kernel_matmul_cuda_kernel_bwd<block_size, thread_dim, per_thread, kernel_function_type, num_params><<<blocks, threads, shared>>>(
        x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
        x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
        rhs.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
        params.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
...
torch::Tensor _lmc_matmul_cuda(
    ...
    const dim3 blocks {
        (block_size + num_threads_x1 - 1) / num_threads_x1,
        (x1.size(0) + block_size - 1) / block_size,
        num_blocks_x2
    };
...
template torch::Tensor kernel_matmul_cuda<RBF, RBF_NUM_PARAMS>(
    torch::Tensor x1,
    torch::Tensor x2,
    torch::Tensor rhs,
    torch::Tensor params,
...
template torch::Tensor kernel_matmul_cuda<SPECTRAL, SPECTRAL_NUM_PARAMS>(
    torch::Tensor x1,
    torch::Tensor x2,
    torch::Tensor rhs,
    torch::Tensor params,
...
template torch::Tensor kernel_bilinear_derivative_cuda<LOCALLY_PERIODIC, LOCALLY_PERIODIC_NUM_PARAMS>(
    torch::Tensor x1,
    torch::Tensor x2,
    torch::Tensor left_vecs,
    torch::Tensor right_vecs,
...
template std::array<torch::Tensor, 2> kernel_weighted_bilinear_derivative_cuda<LOCALLY_PERIODIC, LOCALLY_PERIODIC_NUM_PARAMS>(
    torch::Tensor x1,
    torch::Tensor x2,
    torch::Tensor left_vecs,
    torch::Tensor right_vecs,
...
template std::array<torch::Tensor, 2> kernel_weighted_bilinear_derivative_cuda<SPECTRAL, SPECTRAL_NUM_PARAMS>(
    torch::Tensor x1,
    torch::Tensor x2,
    torch::Tensor left_vecs,
    torch::Tensor right_vecs,
...
template torch::Tensor kernel_matmul_cuda_bwd<RBF, RBF_NUM_PARAMS>(
    torch::Tensor x1,
    torch::Tensor x2,
    torch::Tensor rhs,
    torch::Tensor params,
...
template torch::Tensor lmc_matmul_cuda<RBF, RBF_NUM_PARAMS>(
    torch::Tensor x1,
    torch::Tensor x2,
    torch::Tensor rhs,
    torch::Tensor params,
