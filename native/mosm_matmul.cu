...
__global__ void mosm_vecmul_cuda_kernel(
    const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> x1,  // (samples of task1)
    const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> params1,  // (NUM_PARAMS, batch)
    const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> x2,  // (samples of all tasks)
    const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> params2,  // (tasks, NUM_PARAMS, batch)
    const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> rhs,  // (samples)
    const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> start,  // (row_blocks, tasks)
    const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> end,  // (row_blocks, tasks)
    torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> out  // (batch / b_block_size, samples)
) {
    ...
    static_assert(m_block_size % num_threads_x == 0, "m_block_size must be divisible by num_threads");
    ...
    static_assert(num_threads_x % b_block_size == 0, "num_threads must be divisible by b_block_size");
    const int index2_m_base = threadIdx.x / b_block_size;
    const int index2_m_num_threads = num_threads_x / b_block_size;
    static_assert(m_block_size % index2_m_num_threads == 0, "m_block_size must be divisible by index2_m_num_threads");
    ...
    static_assert(n_block_size % num_threads_x == 0, "n_block_size must be divisible by num_threads");
...
__global__ void mosm_matmul_cuda_kernel(
    ...
    const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> start,  // (row_blocks, tasks)
    const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> end,  // (row_blocks, tasks)
    torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> out  // (batch / b_block_size, samples, k)
) {
        ...
        for(int n_base = start_m; n_base < end_m; n_base += n_block_size) {
            ...
            for(int i = 0; i < n_block_size; i++) {
                ...
                for(int j = 0; j < load_kernel_m_per_thread; j++) {
                    ...
                    // Reduce kernel values over b_block_size
                    for(int offset = b_block_size / 2; offset > 0; offset /= 2) {
                        kernel_value += __shfl_down_sync(0xffffffff, kernel_value, offset, b_block_size);
                    }
...
__global__ void mosm_matmul_bwd_cuda_kernel(
    ...
    const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> start,  // (row_blocks, tasks)
    const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> end,  // (row_blocks, tasks)
    const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> out_grad,  // (samples of task 1, k)
    torch::PackedTensorAccessor32<float, 4, torch::RestrictPtrTraits> params1_grad,  // (NUM_PARAMS, batch, gridDim.x, blockDim.x)
    torch::PackedTensorAccessor32<float, 5, torch::RestrictPtrTraits> params2_grad  // (tasks, NUM_PARAMS, batch, ...)
) {
    // This kernel cannot sum over the batch dimension in advance, because we require the gradient per batch element.
    // As such, we do not have threads over multiple batch entries inside one block.
    ...
    __shared__ std::array<std::array<std::array<float, m_block_size>, n_block_size>, NUM_PARAMS> grads1_shm;
    __shared__ std::array<std::array<std::array<float, m_block_size>, n_block_size>, NUM_PARAMS> grads2_shm;
    __shared__ std::array<std::array<float, k_size>, n_block_size> rhs_shm;
    ...
    // Calculate indices for matmul.
    static_assert(num_threads_x % matmul_k_threads == 0, "num_threads must be divisible by matmul_k_threads");
    const int matmul_m_threads = num_threads_x / matmul_k_threads;
    static_assert(m_block_size % matmul_m_threads == 0, "m_block_size must be divisible by matmul_m_threads");
    const int matmul_m_per_thread = m_block_size / matmul_m_threads;
                ...
                for(int j = 0; j < load_kernel_m_per_thread; j++) {
                    ...
                    if(n < end_m && m < m_size) {
                        ...
                        grads1_reg[0] = sigma_grad_factor_a * (
                            ...
                            - sigma_grad_sum_c * sigma2_sq
                            + sigma_grad_sum_d * sigma2
                        );
                        grads2_reg[0] = sigma_grad_factor_a * (
                            sigma_grad_factor_b * (sigma_grad_sum_a * sigma1_sq + sigma_grad_sum_b)
                            - sigma_grad_sum_c * sigma1_sq
                            - sigma_grad_sum_d * sigma1
                        );
                        ...
                        grads2_reg[3] = alpha * exp_term * (cos_term * tau_theta * sigma + sin_term * mu);
...
__global__ void mosm_bilinear_derivative_cuda_kernel(
    ...
    const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> start,  // (row_blocks, tasks)
    const torch::PackedTensorAccessor32<int, 2, torch::RestrictPtrTraits> end,  // (row_blocks, tasks)
    const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> left_vectors,  // (k, samples of task 1)
    const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> right_vectors,  // (k, samples of task 2)
    torch::PackedTensorAccessor32<float, 5, torch::RestrictPtrTraits> out1,  // (tasks2, NUM_PARAMS, batch, grid.x, threads)
    torch::PackedTensorAccessor32<float, 5, torch::RestrictPtrTraits> out2  // (tasks2, NUM_PARAMS, batch, grid.x, threads)
) {
    // Index calculations
    const int num_threads = num_threads_split * num_threads_split;
    static_assert(block_size % num_threads_split == 0, "block_size must be divisible by num_threads_split");
    ...
    for(int n_base = start_m; n_base < end_m; n_base += block_size) {
        ...
        for(int k_base = 0; k_base < k_size; k_base += load_size) {
            ...
            for(int step = 0; step < load_size; step++) {
                ...
                for(int i = 0; i < per_thread_split; i++) {
                    left_vectors_reg[i] = left_vectors_shm[step][i * num_threads_split + split_index1];
                }
                std::array<float, per_thread_split> right_vectors_reg;
                for(int i = 0; i < per_thread_split; i++) {
                    right_vectors_reg[i] = right_vectors_shm[step][i * num_threads_split + split_index2];
                }
        ...
        for(int j = 0; j < per_thread_split; j++) {
            ...
            for(int i = 0; i < per_thread_split; i++) {
                ...
                const float mu_grad_sum_a = 0.5f * cos_term * mu_diff;
                const float mu_grad_sum_b = sin_term * tau_theta;
                const float sigma1_grad = sigma_grad_factor_a * (
                    sigma_grad_factor_b * (sigma_grad_sum_a * sigma2_sq + sigma_grad_sum_b)
                    - sigma_grad_sum_c * sigma2_sq
                    + sigma_grad_sum_d * sigma2
                );
                const float sigma2_grad = sigma_grad_factor_a * (
                    sigma_grad_factor_b * (sigma_grad_sum_a * sigma1_sq + sigma_grad_sum_b)
                    - sigma_grad_sum_c * sigma1_sq
                    - sigma_grad_sum_d * sigma1
                );
                ...
                const float theta2_grad = alpha * exp_term * (cos_term * tau_theta * sigma + sin_term * mu);
...
__global__ void mosm_dense_cuda_kernel(
    ...
    for(int i = 0; i < per_thread; i++) {
        ...
        if(global_i1 < index1.size(0)) {
            ...
            const int index1_block = (index1_reg - tasks[task1[i]]) / block_size + tasks_block[task1[i]];
        ...
        for(int i = 0; i < per_thread_split; i++) {
            for(int j = 0; j < per_thread_split; j++) {
                ...
                if(in_range) {
                    ...
                    const float sigma = 2.0f * sigma1[i] * sigma2[j] * sigma_inv;
                    const float mu = (sigma1[i] * mu2[j] + sigma2[j] * mu1[i]) * sigma_inv;
                    const float w = w1[i] * w2[j] * exp(-0.25f * (mu1[i] - mu2[j]) * (mu1[i] - mu2[j]) * sigma_inv);
                    const float theta = theta1[i] - theta2[j];
                    const float phi = phi1[i] - phi2[j];
                    const float sqrt_term = sqrt(2.0f * CONSTANT_PI_F * sigma);
                    const float alpha = w * sqrt_term;
                    ...
                    const float sigma1_grad = sigma_grad_factor_a * (
                        sigma_grad_factor_b * (sigma_grad_sum_a * sigma2_sq + sigma_grad_sum_b)
                        - sigma_grad_sum_c * sigma2_sq
                        + sigma_grad_sum_d * sigma2[j]
                    );
                    const float sigma2_grad = sigma_grad_factor_a * (
                        sigma_grad_factor_b * (sigma_grad_sum_a * sigma1_sq + sigma_grad_sum_b)
                        - sigma_grad_sum_c * sigma1_sq
                        - sigma_grad_sum_d * sigma1[i]
                    );
                    const float mu1_grad = -mu_grad_factor * (mu_grad_sum_a + mu_grad_sum_b * sigma2[j]);
                    const float mu2_grad = -mu_grad_factor * (-mu_grad_sum_a + mu_grad_sum_b * sigma1[i]);
...
torch::Tensor mosm_vecmul_cuda(
    torch::Tensor x1,
    torch::Tensor params1,
    torch::Tensor x2,
    torch::Tensor params2,
    torch::Tensor rhs,
    torch::Tensor start,
    torch::Tensor end,
    torch::Tensor tasks,
    ...
    for(int task = 0; task < t; task++) {
        ...
        #ifdef PRINT_SIZE
        printf("task: %d (%d - %d / %d - %d)\n", task, task_start, task_end, task_block_start, task_block_end);
        ...
        mosm_vecmul_cuda_kernel<b_block_size, m_block_size, num_threads, n_block_size><<<blocks, threads>>>(
...
torch::Tensor mosm_matmul_cuda(
    ...
    for(int task = 0; task < t; task++) {
        ...
        mosm_matmul_cuda_kernel<b_block_size, m_block_size, num_threads, n_block_size, k_size><<<blocks, threads>>>(
            x1_chunk.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
            params1_chunk.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
            x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
            params2.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
...
std::array<torch::Tensor, 2> mosm_matmul_bwd_cuda(
    torch::Tensor x1,
    torch::Tensor params1,
    torch::Tensor x2,
    torch::Tensor params2,
    ...
    for(int task = 0; task < t; task++) {
        ...
        auto grad1_chunk = torch::zeros({NUM_PARAMS, params1.size(2), blocks.x, num_threads}, out_opts);
        auto grad2_chunk = torch::zeros({params2.size(0), NUM_PARAMS, params2.size(2), blocks.x, num_threads}, out_opts);

        mosm_matmul_bwd_cuda_kernel<matmul_k_threads, m_block_size, num_threads, n_block_size, k_size><<<blocks, threads>>>(
            x1_chunk.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
            params1_chunk.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
            x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
            params2.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
...
std::array<torch::Tensor, 2> mosm_bilinear_derivative_cuda(
    torch::Tensor x1,
    torch::Tensor params1,
    torch::Tensor x2,
    torch::Tensor params2,
    ...
    for(int task = 0; task < t; task++) {
        ...
        const auto start_chunk = start.slice(0, task_block_start, task_block_end);
        const auto end_chunk = end.slice(0, task_block_start, task_block_end);
        const auto left_vectors_t_chunk = left_vectors.slice(0, task_start, task_end).transpose(0, 1).contiguous();
...
torch::Tensor mosm_dense_cuda(
    ...
    mosm_dense_cuda_kernel<block_size, num_threads_split, per_thread, num_tasks><<<blocks, threads>>>(
...
std::array<torch::Tensor, 2> mosm_dense_bwd_cuda(
    torch::Tensor x1,
    torch::Tensor params1,
    torch::Tensor x2,
    torch::Tensor params2,
    ...
    const auto out_opts = torch::TensorOptions()
        ...
        .layout(x1.layout())
        .device(x1.device());
    auto out1 = torch::zeros({params1.size(0), params1.size(1), params1.size(2), blocks.y, blocks.x, threads.x}, out_opts);
    auto out2 = torch::zeros({params2.size(0), params2.size(1), params2.size(2), blocks.y, blocks.x, threads.x}, out_opts);

    mosm_dense_bwd_cuda_kernel<block_size, num_threads_split, per_thread, num_tasks><<<blocks, threads>>>(
        x1.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
        params1.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
        x2.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
        params2.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
